...
export async function verifyUserIsInSection(cookies: Cookies, userID: string, sectionID: string): Promise<void> {
...
export async function verifyUserIsApartOfInstructionalTeam(cookies: Cookies, userID: string, sectionID: string): Promise<void> {
	const usersMembers = await getUsersSectionMembers(userID);
	if (!usersMembers) {
		const errorMessage = "Failed to get user's section members.";
		const message = {
	...
	if (!member || (member.member_type !== SectionMemberType.TA &&
...
export async function verifyUserIsMember(cookies: Cookies, userID: string, memberID: string): Promise<void> {
	const usersMembers = await getUsersSectionMembers(userID);
	if (!usersMembers) {
		const errorMessage = "Failed to get user's section members.";
		const message = {
