...
<Modal
	...
	<form method="POST" action={`/courses/${member.section.course.id}/sections/${member.section.id}?/updateSectionMember`} id="member-form">
			...
			<dl class="-my-3 divide-y divide-gray-400 text-sm">
				...
				{#if member.member_type === SectionMemberType.Student}
					<div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4 items-center">
						...
						<dd class="text-gray-700 sm:col-span-2">
							<input type="checkbox" class="toggle toggle-primary" id="is-restricted" name="is-restricted" checked={member.is_restricted} />
						</dd>
					</div>
				{/if}
			</dl>
		...
		<div class="flex justify-between">
			<form method="POST" action={`/courses/${member.section.course.id}/sections/${member.section.id}?/deleteSectionMember`} on:submit={confirmRemoval}>
				<input type="hidden" id="member-id" name="member-id" value={member.id}>
				<button type="submit" class="btn btn-error">Remove</button>
			</form>
			<button type="submit" class="btn btn-primary text-white" form="member-form">Save Changes</button>
		</div>
	</form>
</Modal>
