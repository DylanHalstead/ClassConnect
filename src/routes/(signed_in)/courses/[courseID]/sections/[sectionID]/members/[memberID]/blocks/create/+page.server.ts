...
import { getMembersAppointmentBlocks, createAppointmentBlock, deleteAppointmentBlocks } from '$lib/db/appointmentBlocks';
...
export const actions: Actions = {
    ...
    if (!member || (member.member_type !== SectionMemberType.TA && member.member_type !== SectionMemberType.Instructor)) {
    ...
    for (const block of possiblyConflictingBlocks){
      ...
      if ((startMillis < blockStartMillis && endMillis > blockStartMillis) ||
    ...
    if (mergeableBlocks.length > 0) {
      ...
      const earliestStart = new Date(Math.min(...mergeableBlocks.map(block => block.start_time.getTime()), start.getTime()));
      const latestEnd = new Date(Math.max(...mergeableBlocks.map(block => block.start_time.getTime() + block.duration), end.getTime()));
